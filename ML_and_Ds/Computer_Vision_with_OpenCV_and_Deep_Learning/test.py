...
def count_fingers(threshold,hand_segment):
    ...
    conv__hull = cv2.convexHull(hand_segment)

    top = tuple(conv__hull[conv__hull[:,:,1].argmin()[0]])
    bottom = tuple(conv__hull[conv__hull[:, :, 1].argmin()[0]])
    left = tuple(conv__hull[conv__hull[:, :, 0].argmin()[0]])
    right = tuple(conv__hull[conv__hull[:, :, 0].argmin()[0]])
